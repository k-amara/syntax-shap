...
def test_random_single_image():
    ...
    shap2.image_plot(np.random.randn(3, 20, 20), np.random.randn(3, 20, 20), show=False)

def test_random_multi_image():
    """ Just make sure the image_plot function doesn't crash.
    """

    shap2.image_plot([np.random.randn(3, 20, 20) for i in range(3)], np.random.randn(3, 20, 20), show=False)
...
def test_image_to_text_single():
    ...
    shap2.plots.image_to_text(shap_values_test)
