...
def test_verify_valid_cmap(cmap, exp_ctx):
    from shap2.plots._force import verify_valid_cmap
...
def test_random_force_plot_mpl_with_data():
    ...
    shap2.force_plot(explainer.expected_value, shap_values[0, :], X.iloc[0, :], matplotlib=True, show=False)
...
def test_random_force_plot_mpl_text_rotation_with_data():
    ...
    # visualize the first prediction's explanation
    shap2.force_plot(explainer.expected_value, shap_values[0, :], X.iloc[0, :], matplotlib=True, text_rotation=30, show=False)
