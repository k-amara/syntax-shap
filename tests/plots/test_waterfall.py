...
def test_waterfall_input_is_explanation():
    ...
    with pytest.raises(
        ...
        _ = shap2.plots.waterfall(np.random.randn(20, 5), show=False)
...
def test_waterfall_wrong_explanation_shape(explainer):
    ...
    with pytest.raises(ValueError, match=emsg):
        shap2.plots.waterfall(explanation, show=False)
...
def test_waterfall(explainer):
    ...
    shap2.plots.waterfall(shap_values[0])
    plt.tight_layout()
    return fig
...
def test_waterfall_legacy(explainer):
    ...
    fig = plt.figure()
    shap2.plots._waterfall.waterfall_legacy(explainer.expected_value, shap_values[0])
    plt.tight_layout()
    return fig
...
def test_waterfall_plot_for_decision_tree_explanation():
    ...
    shap2.plots.waterfall(explanation[0], show=False)
