...
import shap2
from shap2.utils._exceptions import ConvergenceError, InvalidAction


def create_basic_scenario():
    X = pd.DataFrame({"feature1": np.ones(5), "feature2": np.ones(5), "feature3": np.ones(5)})

    class IncreaseFeature1(shap2.actions.Action):
        """ Sample action.
        """
    ...
    class IncreaseFeature2(shap2.actions.Action):
        """ Sample action.
        """
    ...
    class IncreaseFeature3(shap2.actions.Action):
        """ Sample action.
        """
...
def test_basic_run():
    ...
    optimizer = shap2.ActionOptimizer(passed, possible_actions)
...
def test_bad_action():
    with pytest.raises(InvalidAction):
        shap2.ActionOptimizer(None, [None])
