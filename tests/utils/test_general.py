...
def test_sample_basic(arr):
    ...
    new_arr = shap2.utils.sample(arr, 30, random_state=42)
...
def test_sample_no_op():
    ...
    """
    arr = np.arange(50)
    new_arr = shap2.utils.sample(arr, 100, random_state=42)
...
def test_sample_sampling_without_replacement():
    ...
    """
    arr = np.arange(100)
    new_arr = shap2.utils.sample(arr, 99, random_state=0)
...
def test_sample_can_be_zipped():
    ...
    new_arr1 = shap2.utils.sample(arr1, 75, random_state=42)
    new_arr2 = shap2.utils.sample(arr2, 75, random_state=42)
    new_combined = shap2.utils.sample(combined, 75, random_state=42)
