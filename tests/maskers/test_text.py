...
def test_method_token_segments_pretrained_tokenizer():
    ...
    masker = shap2.maskers.Text(tokenizer)
...
def test_keep_prefix_suffix_tokenizer_parsing():
    ...
    masker_mt = shap2.maskers.Text(tokenizer_mt)
    masker_gpt = shap2.maskers.Text(tokenizer_gpt)
    masker_bart = shap2.maskers.Text(tokenizer_bart)
...
def test_text_infill_with_collapse_mask_token():
    ...
    masker = shap2.maskers.Text(tokenizer, mask_token='...', collapse_mask_token=True)
...
def test_serialization_text_masker():
    ...
    tokenizer = AutoTokenizer.from_pretrained("distilbert-base-cased", use_fast=False)
    original_masker = shap2.maskers.Text(tokenizer)
    ...
    with tempfile.TemporaryFile() as temp_serialization_file:
        ...
        # deserialize masker
        new_masker = shap2.maskers.Text.load(temp_serialization_file)
...
def test_serialization_text_masker_custom_mask():
    ...
    original_masker = shap2.maskers.Text(tokenizer, mask_token='[CUSTOM-MASK]')
...
def test_serialization_text_masker_collapse_mask_token():
    ...
    tokenizer = AutoTokenizer.from_pretrained("distilbert-base-cased", use_fast=True)
    original_masker = shap2.maskers.Text(tokenizer, collapse_mask_token=True)
