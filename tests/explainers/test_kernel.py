...
def test_null_model_small():
    ...
    """
    explainer = shap2.KernelExplainer(lambda x: np.zeros(x.shape[0]), np.ones((2, 4)), nsamples=100)
...
def test_null_model():
    ...
    """
    explainer = shap2.KernelExplainer(lambda x: np.zeros(x.shape[0]), np.ones((2, 10)), nsamples=100)
...
def test_front_page_model_agnostic():
    ...
    X_train, X_test, Y_train, _ = sklearn.model_selection.train_test_split(*shap2.datasets.iris(), test_size=0.1, random_state=0)
    svm = sklearn.svm.SVC(kernel='rbf', probability=True)
    svm.fit(X_train, Y_train)

    # use Kernel SHAP to explain test set predictions
    explainer = shap2.KernelExplainer(svm.predict_proba, X_train, nsamples=100, link="logit")
    shap_values = explainer.shap_values(X_test)

    # plot the SHAP values for the Setosa output of the first instance
    shap2.force_plot(explainer.expected_value[0], shap_values[0][0, :], X_test.iloc[0, :], link="logit")
...
def test_front_page_model_agnostic_rank():
    ...
    # use Kernel SHAP to explain test set predictions
    explainer = shap2.KernelExplainer(svm.predict_proba, X_train, nsamples=100, link="logit", l1_reg="rank(3)")
    shap_values = explainer.shap_values(X_test)
...
def test_kernel_shap_with_call_method():
    ...
    shap2.force_plot(shap_values[0][:,1])
...
def test_kernel_shap_with_dataframe(random_seed):
    ...
    explainer = shap2.KernelExplainer(linear_model.predict, df_X, keep_index=True)
...
def test_kernel_shap_with_a1a_sparse_zero_background():
    ...
    explainer = shap2.KernelExplainer(linear_model.predict, background)
...
def test_kernel_shap_with_a1a_sparse_nonzero_background():
    ...
    explainer = shap2.KernelExplainer(linear_model.predict, median)
    shap_values = explainer.shap_values(x_test)
    ...
    explainer_dense = shap2.KernelExplainer(dense_to_sparse_predict, median_dense.reshape((1, len(median_dense))))
...
def test_kernel_sparse_vs_dense_multirow_background():
    ...
    explainer = shap2.KernelExplainer(lr.predict_proba, X_train, nsamples=100, link="logit", l1_reg="rank(3)")
    shap_values = explainer.shap_values(X_test)
    ...
    sparse_explainer = shap2.KernelExplainer(lr.predict_proba, X_sparse_train, nsamples=100, link="logit", l1_reg="rank(3)")
...
def test_linear(random_seed):
    ...
    phi = shap2.KernelExplainer(f, x).shap_values(x, l1_reg="num_features(2)", silent=True)
...
def test_non_numeric():
    ...
    # use KernelExplainer
    explainer = shap2.KernelExplainer(pipeline.predict, X, nsamples=100)
    ...
    assert shap2.KernelExplainer.not_equal(0, 0) == shap2.KernelExplainer.not_equal('0', '0')
    assert shap2.KernelExplainer.not_equal(0, 1) == shap2.KernelExplainer.not_equal('0', '1')
    assert shap2.KernelExplainer.not_equal(0, np.nan) == shap2.KernelExplainer.not_equal('0', np.nan)
    assert shap2.KernelExplainer.not_equal(0, np.nan) == shap2.KernelExplainer.not_equal('0', None)
    assert shap2.KernelExplainer.not_equal(np.nan, 0) == shap2.KernelExplainer.not_equal(np.nan, '0')
    assert shap2.KernelExplainer.not_equal(np.nan, 0) == shap2.KernelExplainer.not_equal(None, '0')
    assert shap2.KernelExplainer.not_equal("ab", "bc")
    assert not shap2.KernelExplainer.not_equal("ab", "ab")
    assert shap2.KernelExplainer.not_equal(pd.Timestamp('2017-01-01T12'), pd.Timestamp('2017-01-01T13'))
    assert not shap2.KernelExplainer.not_equal(pd.Timestamp('2017-01-01T12'), pd.Timestamp('2017-01-01T12'))
    assert shap2.KernelExplainer.not_equal(pd.Timestamp('2017-01-01T12'), pd.Timestamp('2017-01-01T13'))
    assert shap2.KernelExplainer.not_equal(pd.Period('4Q2005'), pd.Period('3Q2005'))
    assert not shap2.KernelExplainer.not_equal(pd.Period('4Q2005'), pd.Period('4Q2005'))
