...
import shap2
from shap2 import DeepExplainer
...
def test_tf_keras_mnist_cnn(random_seed):
    ...
    e = shap2.DeepExplainer((model.layers[0].input, model.layers[-1].input), x_train[inds, :, :])
...
def test_tf_keras_linear():
    ...
    # explain
    e = shap2.DeepExplainer((model.layers[0].input, model.layers[-1].output), x)
...
def test_tf_keras_imdb_lstm(random_seed):
    ...
    # writer.close()
    e = shap2.DeepExplainer((mod.layers[0].input, mod.layers[-1].output), background)
...
def test_pytorch_mnist_cnn(torch_device, interim):
    ...
    if interim:
        e = shap2.DeepExplainer((model, model.conv_layers[0]), next_x_random_choices)
    else:
        e = shap2.DeepExplainer(model, next_x_random_choices)
...
def test_pytorch_multiple_inputs(torch_device, disconnected):
    ...
    e = shap2.DeepExplainer(model, background)
