...
def test_front_page_xgboost():
    ...
    explainer = shap2.GPUTreeExplainer(model)
...
def test_gpu_tree_explainer_shap(task, feature_perturbation):
    model, X, _ = task
    gpu_ex = shap2.GPUTreeExplainer(model, X, feature_perturbation=feature_perturbation)
    ex = shap2.TreeExplainer(model, X, feature_perturbation=feature_perturbation)
...
def test_gpu_tree_explainer_shap_interactions(task, feature_perturbation):
    model, X, margin = task
    ex = shap2.GPUTreeExplainer(model, X, feature_perturbation=feature_perturbation)
