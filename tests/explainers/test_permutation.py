...
def test_exact_second_order(random_seed):
    ...
    shap_values = shap2.explainers.PermutationExplainer(model, np.zeros((1,5)))(data)
...
def test_tabular_single_output_auto_masker():
    ...
    common.test_additivity(shap2.explainers.PermutationExplainer, model.predict, data, data)

def test_tabular_multi_output_auto_masker():
    model, data = common.basic_xgboost_scenario(100)
    common.test_additivity(shap2.explainers.PermutationExplainer, model.predict_proba, data, data)

def test_tabular_single_output_partition_masker():
    model, data = common.basic_xgboost_scenario(100)
    common.test_additivity(shap2.explainers.PermutationExplainer, model.predict, shap2.maskers.Partition(data), data)

def test_tabular_multi_output_partition_masker():
    model, data = common.basic_xgboost_scenario(100)
    common.test_additivity(shap2.explainers.PermutationExplainer, model.predict_proba, shap2.maskers.Partition(data), data)

def test_tabular_single_output_independent_masker():
    model, data = common.basic_xgboost_scenario(100)
    common.test_additivity(shap2.explainers.PermutationExplainer, model.predict, shap2.maskers.Independent(data), data)

def test_tabular_multi_output_independent_masker():
    model, data = common.basic_xgboost_scenario(100)
    common.test_additivity(shap2.explainers.PermutationExplainer, model.predict_proba, shap2.maskers.Independent(data), data)

def test_serialization():
    model, data = common.basic_xgboost_scenario()
    common.test_serialization(
        shap2.explainers.PermutationExplainer, model.predict, data, data,
