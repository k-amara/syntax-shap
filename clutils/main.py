...
if not os.path.exists("/cluster/work/zhang/kamara/checkpoints"):
    os.makedirs("/cluster/work/zhang/kamara/checkpoints")
...
def trash_expe(expe, trash_dir):
    ...
    assert expe.startswith(
        ckpt_default()
    ), "Directory should start with default checkpoint"
    assert (
        len(dirs) - base_level == 3
    ), "Experiment should be 2 levels below main checkpoint directory"
...
def stool_stress(partition):
    ...
    """
    STRING = (
        "bqueues " + partition + ' --format "%t %u %b" '
        '| grep "^R" '
        '| grep -v "null" '
        "| cut -b3- "
        '| sed -e "s/gpu://g" '
    ...
    STRING = (
        "bqueues " + partition + ' --format "%t %u %C" '
        '| grep "^R" '
        '| grep -v "null" '
        "| cut -b3- "
        '| sed -e "s/gpu://g" '
...
if args.command == "sweep":
    assert os.path.exists(args.grid), (
        "Config file %s does not exist. Are you in the right repository ?" % args.grid
    )
    ...
    if "machine" in config:
        if "time" in config["machine"]:
            sbatch += "--time=%s " % config["machine"]["time"]  # In minutes
        # Task is a node
        # if "num_gpu" in config["machine"]:
            # sbatch += "-p gpu "
            # sbatch += "--gres=gpu:%s " % config["machine"]["num_gpu"]
        #if "gpu-model" in config["machine"]:
            #sbatch += "--gpus=%s:1 " % config["machine"]["gpu-model"] #--gpus=rtx_3090:1
        if "gpus" in config["machine"]:
            sbatch += "--gpus=%s " % config["machine"]["gpus"]
        if "gres" in config["machine"]:
            sbatch += "--gres=%s " % config["machine"]["gres"] #20g

        if "cpus-per-task" in config["machine"]:
            sbatch += "-n %s " % config["machine"]["cpus-per-task"]
        if "mem-per-cpu" in config["machine"]:
            sbatch += "--mem-per-cpu=%s " % config["machine"]["mem-per-cpu"]
        sbatch += "< "
    ...
    if os.path.exists(log_dir):
        ...
        print(
            f"- (default) Abort. Experiment folder will be kpet untouched and new experiments will not be run."
        )
    ...
    param_values = {
        k: sorted(list(set([d[k] for d in paramset if k in d]))) for k in param_names
    }
    ...
    params_to_index = [
        ...
        if len(values) >= 2
        and (
            any(["/" in v for v in values if type(v) is str])
            or any([len(str(v)) >= 20 for v in values])
        )
    ]
    ...
    if args.array:
        ...
        with open(filename, "w") as f:
            f.write(
                cmdPre(
                    ...
                    log_stderr,
                    filename,
                    num_expes=len(paramset),
                    pooling=args.pooling,
                )
            )

        with open(join(expdir, "params.txt"), "w") as f, open(
            join(expdir, "commands.txt"), "w"
        ) as f_cmd:
            for params in paramset:
                ...
                if args.dest_arg:
                    if os.path.exists(join(expdir, ext)):
                        print(
                            "WARNING: %s already exists (whatever is in there will probably be overwritten by experiment)"
                            % join(expdir, ext)
                        )
                    # else:
                    # os.makedirs(join(expdir, ext))
                    dest_name = (
                        config["meta"]["dest-name"]
                        if "dest-name" in config["meta"]
                        else "dest"
                    )
        ...
        for i_param, params in enumerate(paramset):
            ...
            with open(filename, "w") as f:
                f.write(
                    cmdPre(config, params, name + ext, log_stdout, log_stderr, filename)
                )
            ...
            if args.launch_parallel:
                ...
                start = time.time()
                if args.launch:
                    r = subprocess.run([sbatch + filename], shell=True, check=True, capture_output=True)
...
# Shamelessly copy-pasted from stool
    ...
    print(
        run_command(
            "bqueues -u "
            + users
            + ' -o "%90j | %9A | %4t | %12b | %4C | %10m | %10M | %N | %P | %u"'
        )
    )
