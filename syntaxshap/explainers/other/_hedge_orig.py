...
class HEDGEOrig(Explainer):
    ...
    def explain_row(self, *row_args, max_evals, main_effects, error_bounds, batch_size, outputs, silent):
        ...
        inputs = self.tokenizer(row_args[0], add_special_tokens=True, return_tensors='pt', padding=True, truncation=True)

        #print('score ', self.model_init(**inputs)[1])
        score = self.model.get_probs(inputs)[1].detach().cpu().numpy()
        score_norm = (np.exp(score) / np.sum(np.exp(score), axis=1))
        self.pred_label = np.argmax(score_norm)
        print('self_label', self.pred_label)
        self.output = []
        ...
        temp = {'input_ids': torch.unsqueeze(mask_input, 0).to(self.device),
                'attention_mask': torch.unsqueeze(mask_attention, 0).to(self.device),
                'token_type_ids': torch.unsqueeze(mask_type, 0).to(self.device),
                'labels': self.pred_label}
        #score = self.model_init(**temp)[1].detach().cpu().numpy()
        score = self.model.get_probs(temp)[1].detach().cpu().numpy()
        score_norm = (np.exp(score) / np.sum(np.exp(score), axis=1))
        self.bias = score_norm[0][self.pred_label]
        
        self.compute_shapley_hier_tree(self.model, inputs, 2)
        phrase_list, score_list = self.get_importance_phrase()
        for fea in self.hier_tree[self.max_level]:
            print('fea:', fea)
            
        vals = np.array([fea[1] for fea in self.hier_tree[self.max_level]])
        print('vals:', vals)
        print('num tokens', len(inputs['input_ids'][0]))
        print('len(vals)', len(vals))
        row_values = phrase_list
    ...
    def set_contribution_func(self, model, fea_set, inputs):
        ...
        for fea_idx in fea_set:
            if type(fea_idx) == int:
                ...
                mask_input[fea_idx] = input[fea_idx]
                mask_attention[fea_idx]
            else:
                for idx in fea_idx:
                    ...
                    mask_attention[idx] = 1 
        temp = {'input_ids': torch.unsqueeze(mask_input, 0).to(self.device),
                'attention_mask': torch.unsqueeze(mask_attention, 0).to(self.device),
                'token_type_ids': torch.unsqueeze(mask_type, 0).to(self.device),
                'labels': self.pred_label}
        # send the mask_input into model
        score = model.get_probs(temp)[1].detach().cpu().numpy()
        score_norm = (np.exp(score) / np.sum(np.exp(score), axis=1))
