...
class HEDGE(Explainer):

    def __init__(self, model, masker, model_init, *, output_names=None, link=links.identity, linearize_link=True,
        ...
        masker : function or numpy.array or pandas.DataFrame or tokenizer
            ...
            functions are available in shap such as maksers.Image for images and maskers.Text
            for text.
        ...
        Examples
        --------
        See `Partition explainer examples <https://shap2.readthedocs.io/en/latest/api_examples/explainers/PartitionExplainer.html>`_
        """
        ...
        if getattr(self.masker, "clustering", None) is None:
            raise ValueError("The passed masker must have a .clustering attribute defined! Try maskers.Partition(data) for example.")
    ...
    def owen(self, fm, p00, p11, max_evals, output_indexes, fixed_context, batch_size, silent):
        ...
        base_value = self.get_contribution(m00)
        #f11 = fm(~m00.reshape(1,-1))[0]
        ...
        if output_indexes is not None:
            ...
            p00 = p00[output_indexes]
            p11 = p11[output_indexes]

        q = queue.PriorityQueue()
        q.put((0, 0, (ind, 1.0, p00, p11, m00)))
        eval_count = 0
        ...
        while not q.empty():
            ...
            if eval_count >= max_evals:
                while not q.empty():
                    ind, weight, p00, p11, m00 = q.get()[2]
                    self.dvalues[ind] += (p11 - p00) * weight
                break
            ...
            while not q.empty() and len(batch_masks) < batch_size and eval_count + len(batch_masks) < max_evals:
                ...
                batch_args.append((m00, m10, m01, p00, p11, ind, lind, rind, weight))
            ...
            if len(batch_args) > 0:
                ...
                for mask in batch_masks:
                    pout = self.get_contribution(mask)
                    batch_pout.append(pout)
                if output_indexes is not None:
            ...
            for i in range(len(batch_args)):
                ...
                m00, m10, m01, p00, p11, ind, lind, rind, weight = batch_args[i]
                ...
                if fixed_context is None:
                    ...
                    self.dvalues[ind] += (p11 - p10 - p01 + p00) * weight # leave the interaction effect on the internal node
                elif fixed_context == 1:
                    self.dvalues[ind] -= (p11 - p10 - p01 + p00) * weight # leave the interaction effect on the internal node

                if fixed_context is None or fixed_context == 0:
                    # recurse on the left node with zero context
                    args = (lind, weight, p00, p10, m00)
                    q.put((-np.max(np.abs(p10 - p00)) * new_weight, np.random.randn(), args))

                    # recurse on the right node with zero context
                    args = (rind, weight, p00, p01, m00)
                    q.put((-np.max(np.abs(p01 - p00)) * new_weight, np.random.randn(), args))

                if fixed_context is None or fixed_context == 1:
                    # recurse on the left node with one context
                    args = (lind, weight, p01, p11, m01)
                    q.put((-np.max(np.abs(p11 - p01)) * new_weight, np.random.randn(), args))

                    # recurse on the right node with one context
                    args = (rind, weight, p10, p11, m10)
                    q.put((-np.max(np.abs(p11 - p10)) * new_weight, np.random.randn(), args))
        ...
        while not q.empty():
            ...
            for i in range(len(batch_args)):
                ...
                if context is None or context == 0 or ignore_context:
                    ...
                    q.put((-np.max(np.abs(f10 - f00)+1e-6) * new_weight, np.random.randn(), args))
                    ...
                    q.put((-np.max(np.abs(f01 - f00)+1e-6) * new_weight, np.random.randn(), args))
                ...
                if context is None or context == 1 or ignore_context:
                    ...
                    q.put((-np.max(np.abs(f11 - f01)+1e-6) * new_weight, np.random.randn(), args))
                    ...
                    q.put((-np.max(np.abs(f11 - f10)+1e-6) * new_weight, np.random.randn(), args))
